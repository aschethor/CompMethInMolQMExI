...
void Integrate(double Step, VECTOR *Momentum) {
    ...
    // verlet integrator (not velocity verlet!)
    UKinetic = 0.0;
    /*
    Force();
    ...
    for (i = 0; i < NumberOfParticles; i++) {
        Positions[i].x = NewPositions[i].x = Positions[i].x + Velocities[i].x * Deltat + Forces[i].x / 2 * SQR(Deltat);
        Positions[i].y = NewPositions[i].y = Positions[i].y + Velocities[i].y * Deltat + Forces[i].y / 2 * SQR(Deltat);
        Positions[i].z = NewPositions[i].z = Positions[i].z + Velocities[i].z * Deltat + Forces[i].z / 2 * SQR(Deltat);

        Velocities[i].x += Forces[i].x / 2 * Deltat;
        Velocities[i].y += Forces[i].y / 2 * Deltat;
        Velocities[i].z += Forces[i].z / 2 * Deltat;
    }
    Force();
    for (i = 0;i < NumberOfParticles; i++) {
        Velocities[i].x += Forces[i].x / 2 * Deltat;
    ...
    if (Step < NumberOfInitializationSteps)
        Scale = sqrt(
                Temperature * (3.0 * NumberOfParticles - 3.0) / (2.0 * UKinetic));            //** for initialization
    else
        Scale = 1.0;
    ...
    for (i = 0; i < NumberOfParticles; i++) {
        ...
        /*
        NewPositions[i].x = OldPositions[i].x + Velocities[i].x * 2 * Deltat;//I don't understand this part...
        NewPositions[i].y = OldPositions[i].y + Velocities[i].y * 2 * Deltat;//does it actually belong to pos Verlet?!
        NewPositions[i].z = OldPositions[i].z + Velocities[i].z * 2 * Deltat;
        */

        PositionsNONPDB[i].x +=
                NewPositions[i].x - Positions[i].x; //PositionsNONPDB=NewPosition but we will not apply PBC
        PositionsNONPDB[i].y += NewPositions[i].y - Positions[i].y;
        PositionsNONPDB[i].z += NewPositions[i].z - Positions[i].z;

        UKinetic += 0.5 * (SQR(Velocities[i].x) + SQR(Velocities[i].y) +
                           SQR(Velocities[i].z));   //*** for after Inizialization
    ...
    Pressure += 2.0 * UKinetic * NumberOfParticles /
                (CUBE(Box) * (3.0 * NumberOfParticles - 3)); //Thi final -3 is due to fix COM: there are (3*N-3) dof
}
